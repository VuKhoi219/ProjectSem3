...
public class InsuranceContractsController : ControllerBase
{
    ...
    [HttpGet("admin/detail/{contractId}/{planId}")]
    public async Task<IActionResult> GetContractDetailAdmin(int contractId, int planId )
    {
        try
        {
            var contract = await _context.InsuranceContracts
                ...
                .Select(c => new
                {
                    ...
                    User = new // Thêm thông tin từ bảng Users
                    {
                      ...
                      CitizenIdentificationCard = c.User.CitizenIdentificationCard
                    },
                    InsurancePlan = c.PlanId == planId
                        ? _context.InsurancePlans
                            .Where(l => l.Id == planId)
